...
def create_graphs(X, y, corr_threshold=0.8):
    n_samples, n_channels, n_time = X.shape
    graphs = []
    for i in range(n_samples):
        x = torch.tensor(X[i], dtype=torch.float32)
        # Compute correlations per channel pair (memory-efficient for small n_channels)
        ...
        for j in range(n_channels):
            for k in range(n_channels):
                if j == k:
                    ...
                    corr_matrix[j,k] = np.corrcoef(X[i,j], X[i,k])[0,1]
        edge_index = []
        for j in range(n_channels):
...
def train_and_evaluate_split(folder, batch_size=1, epochs=20, corr_threshold=0.8, downsample=10):
