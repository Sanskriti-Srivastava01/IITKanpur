...
from torch_geometric.nn.glob import global_sort_pool  # Correct import
...
# 5. GCN + SortPool Model
class EEGGCN_SortPoolNet(nn.Module):
    def __init__(self, node_feat_dim, hidden_dim=32, k=10, num_classes=2):
        super().__init__()
    ...
    def forward(self, data):
        ...
        x = global_sort_pool(x, batch, self.k)  # Use global_sort_pool function
        return self.classifier(x)

# 6. Training and Evaluation
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
k = 10  # Number of nodes to keep after sorting
model = EEGGCN_SortPoolNet(node_feat_dim=epoch_size, k=k).to(device)
