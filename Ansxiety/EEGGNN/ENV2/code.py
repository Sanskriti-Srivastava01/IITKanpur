...
# 1. Sliding Window Epoch Extraction
def load_all_epochs(folder, label, epoch_size=1000, overlap=0.5):
    """
    Load all EEG CSV files in a folder and segment them into epochs using sliding window.
    Returns a list of (epoch_array, label)
    """
    all_epochs = []
...
print(f"Loaded {len(all_data)} epochs (windows), each with shape {all_data[0][0].shape}.")
...
# 4. Train/Test Split (epoch-level)
...
model = EEGGNN(node_feat_dim=epoch_size).to(device)
