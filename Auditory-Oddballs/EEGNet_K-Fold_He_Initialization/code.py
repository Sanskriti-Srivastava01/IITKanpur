...
# ========== 1b. Normalization Check & Option ==========
def check_normalization_scaling(X):
    if X.ndim == 4 and X.shape[1] == 1:
        X = X.squeeze(1)
    mean_per_trial = np.mean(X, axis=(1,2))
    std_per_trial = np.std(X, axis=(1,2))
    ...
    print("- If overall mean is close to 0 and std close to 1, data is normalized.")
    return overall_mean, overall_std

def zscore_trialwise(X):
    if X.ndim == 4 and X.shape[1] == 1:
        X = X.squeeze(1)
    X_norm = (X - np.mean(X, axis=(1,2), keepdims=True)) / (np.std(X, axis=(1,2), keepdims=True) + 1e-8)
    return X_norm[:, np.newaxis, :, :]

print("\n--- Checking normalization before model input ---")
overall_mean, overall_std = check_normalization_scaling(X)
if abs(overall_mean) > 0.1 or abs(overall_std - 1) > 0.1:
    print("Applying z-score normalization trialwise...")
    X = zscore_trialwise(X)
...
class EEGNet(nn.Module):
    ...
    def forward(self, x, return_acts=False):
        ...
        acts['conv1'] = x.detach().cpu()
        x = self.bn1(x)
        x = F.elu(self.conv2(x))
        acts['conv2'] = x.detach().cpu()
        x = self.bn2(x)
        x = self.pool2(x)
        x = F.elu(self.conv3(x))
        acts['conv3'] = x.detach().cpu()
        x = F.elu(self.conv4(x))
        acts['conv4'] = x.detach().cpu()
        x = self.bn3(x)
        x = self.pool3(x)
...
# ========== 4. K-Fold Training, Visualization, Diagnostics ==========
...
for fold, (train_idx, test_idx) in enumerate(skf.split(X, y)):
    ...
    # ========== 5. Visualize Weights After Each Fold ==========
    for name, param in model.named_parameters():
        if 'weight' in name and param.requires_grad:
            plt.figure()
            plt.hist(param.detach().cpu().numpy().flatten(), bins=50)
            plt.title(f'Fold {fold+1} Weight distribution: {name}')
            plt.show()

# ========== 6. Visualization: Metrics, Confusion, ROC ==========
...
# ========== 7. Visualize Intermediate Activations ==========
def visualize_activations(sample, model, channel_idx=0):
    model.eval()
    with torch.no_grad():
        logits, acts = model(sample, return_acts=True)
    # Original EEG
    plt.figure(figsize=(12, 4))
    plt.plot(sample[0,0,channel_idx,:].cpu().numpy())
    plt.title('Original EEG Signal')
    # Layer 1 Activations
    act1 = acts['conv1'].numpy()
    plt.figure(figsize=(12, 8))
    for i in range(16):
        plt.subplot(4, 4, i+1)
        plt.plot(act1[0,i,channel_idx,:])
        plt.title(f'Conv1 Filter {i+1}')
        plt.axis('off')
    plt.suptitle('Layer 1 Activations')
    # Layer 2 Activations
    act2 = acts['conv2'].numpy()
    plt.figure(figsize=(12, 8))
    for i in range(4):
        plt.subplot(2, 2, i+1)
        plt.imshow(act2[0,i,:,:], aspect='auto', cmap='RdBu_r')
        plt.title(f'Conv2 Filter {i+1}')
        plt.xlabel('Time')
        plt.ylabel('Channels')
    plt.suptitle('Layer 2 Activations')

# Visualize for the first test sample of the last fold
sample_input = torch.tensor(X_test[0:1]).to(device)
visualize_activations(sample_input, model)
